#Code was gathered at: https://www.kaggle.com/robertq/language-model-for-sentiment-analysis/notebook
...
class lm(object):
	...
	"""
	def __init__(self, a=0.0001, smooth_method='jelinek_mercer'):
...
for i in range(len(dfs)):
	...
	for senti, words in predictive_words.items():
		...
		for w, frac in words.items():
			...
			if tot_frac == 0.0: ax.bar(x - width/8.0, frac, color = colors[ct], alpha=0.7, width = width)
			else: ax.bar(x - width/8.0, frac, bottom = tot_frac, color = colors[ct], alpha=0.7, width = width)
			ct += 1
			tot_frac += frac
plt.show()
